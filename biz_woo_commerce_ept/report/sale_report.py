...
class SaleReport(models.Model):
    ...
    def woo_sale_report(self):
        ...
        if version_info.get('server_version') == '15.0':
            action = self.env.ref('biz_woo_commerce_ept.woo_action_order_report_all').read()[0]
        else:
            action = self.env.ref('biz_woo_commerce_ept.woo_sale_report_action_dashboard').read()[0]
